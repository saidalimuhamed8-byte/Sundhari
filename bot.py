...
def set_fsub_channel(invite_link: str):
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute("SELECT COALESCE(MAX(id), 0) FROM fsub_channel")
    old_version = cur.fetchone()[0]
    new_version = old_version + 1
    cur.execute("DELETE FROM fsub_channel")
    cur.execute("INSERT INTO fsub_channel (id, invite_link) VALUES (?, ?)", (new_version, invite_link))
    conn.commit()
    conn.close()
    return new_version
...
def get_fsub_channel():
    ...
    row = cur.fetchone()
    conn.close()
    return row if row else (0, None)   # (version, invite_link)
...
async def fsub_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    version = set_fsub_channel(context.args[0])
    await update.message.reply_text(
        f"✅ Force sub channel set to `{context.args[0]}` (version {version})",
        parse_mode="Markdown"
    )
