...
ADMIN_IDS = [8301447343]  # Telegram user IDs of admins
LOG_CHANNEL_ID = -1002871565651  # Telegram channel ID for logs
BATCH_SIZE = 10  # videos per set/page
...
def add_pending_request(user_id, channel_id):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("INSERT OR IGNORE INTO pending_requests (user_id, channel_id) VALUES (?, ?)",
                   (user_id, channel_id))
    conn.commit()
    conn.close()
...
def remove_pending_request(user_id, channel_id):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM pending_requests WHERE user_id=? AND channel_id=?", (user_id, channel_id))
    conn.commit()
    conn.close()
...
def is_pending_request(user_id, channel_id):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("SELECT 1 FROM pending_requests WHERE user_id=? AND channel_id=?", (user_id, channel_id))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("request:"):
        ...
        await query.message.reply_text("✅ Request approved! Sending videos...")
    ...
    # Force join check
        ...
        await query.message.reply_text("📌 Click **Request to Join** to access videos.", parse_mode="Markdown",
...
async def set_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ Force-join channel updated:\nID: `{channel_id}`\nLink: {invite_link}",
                                        parse_mode="Markdown")
    except Exception as e:
